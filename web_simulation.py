    ...
    if (st.button('Summit ') or ('input_list' in st.session_state)):
        ...
        with st.expander('Portfolio2', expanded=False):
            ...
            if st.button('Simulation '):
                ...
                st.session_state.slider = (slider * 0.01).tolist()
                st.session_state.portfolio_port, st.session_state.allocation = backtest.simulation(
                ...
                if 'slider' in st.session_state:
                    ...
                    Anuuual_Vol = round(float(
                        np.std(st.session_state.portfolio_port.pct_change().dropna()) * np.sqrt(annualization) * 100),
                                        2)
                    Anuuual_Sharpe = round(Anuuual_RET / Anuuual_Vol, 2)
                    MDD = round(float(min(st.session_state.drawdown) * 100), 2)
                    Daily_RET = st.session_state.portfolio_port.pct_change().dropna()
                    ...
                    with col_a:
                        ...
                        plt.xlabel("Daily Return", size=12)
                        plt.ylabel("Probability", size=12)
                        plt.xticks(np.arange(Daily_RET.min().round(2), Daily_RET.max().round(2),
                                             (Daily_RET.max().round(2) - Daily_RET.min().round(2)) / 10), fontsize=12)
                        plt.grid(True, alpha=1, linestyle="--")
                        plt.rcParams["figure.figsize"] = [7, 5]
                        # plt.rcParams["figure.dpi"] = 500
