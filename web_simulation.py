    ...
    if (st.button('Summit') or ('input_list' in st.session_state)):
        ...
        with st.expander('Portfolio', expanded=True):
            ...
            if st.button('Simulation'):
                ...
                st.session_state.log_ret = np.log((st.session_state.input_price.iloc[1:] /
                                                 st.session_state.input_price.shift(1).dropna()))
                st.session_state.contribution = (st.session_state.log_ret* (st.session_state.alloc.shift(1).dropna())).dropna().sum(axis=0)
